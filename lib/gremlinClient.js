...
GremlinClient.prototype.buildEdgeQuery_ = function (data) {
    ...
    switch (direction) {
        case 'in': query = templates.targetTemplate + ".next().addEdge('"+label+"', " + templates.sourceTemplate  + ".next()," + templates.edgeTemplate +")";
            break;
        case 'out': query = templates.sourceTemplate + ".next().addEdge('"+label+"', " + templates.targetTemplate + ".next()," +  templates.edgeTemplate +")";
            break;
        default:
